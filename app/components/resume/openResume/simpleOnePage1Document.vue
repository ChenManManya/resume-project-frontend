...
<template>
  <section class="open-simple-one-page-1" :style="{ '--open-primary': documentData.primaryColor }">
    <header class="header">
      <div class="hello">HELLO<span>°</span></div>
      <div class="name">我是{{ documentData.name }}</div>
    </header>
    <div class="base-info">
      <span>城市: {{ documentData.city || '—' }}</span>
      <span>电话: {{ documentData.phoneNumber || '—' }}</span>
      <span>邮箱: {{ documentData.email || '—' }}</span>
    </div>
    <main class="main">
      <aside class="left">
      ...
      <section class="right">
        ...
        <div class="title intro-title">自我评价</div>
...
.open-simple-one-page-1 { width: 100%; min-height: 100%; display: flex; flex-direction: column; background: #fff; }
.header { padding: 18px 38px 10px; }
.hello { font-size: 40px; font-weight: 800; color: var(--open-primary, #2563eb); }
.name { margin-top: 6px; font-size: 28px; font-weight: 700; color: #111827; }
.base-info { display: flex; flex-wrap: wrap; gap: 12px 28px; padding: 0 38px 24px; color: #475569; font-size: 14px; }
